...
8. **(Optional) real AI in local dev too.** Rather than re-typing the key, **pull it down** from Vercel after deploying:
   ...
   ```
   That writes the key into `.env.local` (gitignored — never commit it), so `npm run dev` now uses the real model too. Skip this if testing on the live URL is enough — the key is only ever entered once (on Vercel).

## Facilitator note

Use **one dedicated, spend-capped** Anthropic key for the class, set it **once on Vercel** (`vercel env add`), and **revoke it after**. Students never type it into the code and never enter it twice — `vercel env pull` brings it to local dev if they want real AI there. After revocation the deployed apps fall back to the mock (the route returns a clean error). The spend cap is the real protection — students never see the key.

## Hard rules

- **The key is a server-side env var — NEVER in the browser, the code, or a commit.** No `dangerouslyAllowBrowser`. Set it once on Vercel; `.env.local` is only ever *generated by* `vercel env pull` (gitignored) — never hand-typed or committed.
