...
3. **If the app has a server route that needs a secret** (the `ANTHROPIC_API_KEY` from `add-ai`): the key is **not** in the code — set it on Vercel, then redeploy so it takes effect.
   - **Set the key on the Vercel website (recommended) — PAUSE and hand this step to the student.** Tell them, step by step: open **vercel.com → this project → Settings → Environment Variables**, add a variable with the **Key (name): `ANTHROPIC_API_KEY`** (spell the exact name out — it must match the code), **Value:** the facilitator's shared capped key, **Environment: Production**, then **Save**. **Do not type or paste the key yourself — it's their secret.** **Wait for them to confirm it's saved**, then continue to the redeploy.
   - *Or in the terminal* (still the student pasting): `npx vercel env add ANTHROPIC_API_KEY production` → they paste the key at the prompt.
   - **Redeploy** so the new var is picked up: `npx vercel --prod --yes` (or "Redeploy" from the dashboard). *(Env-var changes only apply to deployments created **after** they're added — the redeploy is required, and "I set the key but it still fails" is almost always a missing redeploy.)*
...
- **Pause for the student to enter the API key themselves** — never type or paste the key for them. Give them the **exact** variable name `ANTHROPIC_API_KEY`, and **redeploy only after they confirm it's saved**.
